...
namespace ESFA.DC.JobQueueManager.ExternalData
{
    public sealed class ScheduleService : IScheduleService
    {
        ...
        public ScheduleService(ILogger logger, IJobQueryService jobQueryService)
        {
            _logger = logger;
            _jobQueryService = jobQueryService;
        }

        public async Task<bool> CanExecuteSchedule(Schedule schedule, DateTime nowUtc, bool removeOldDates)
        {
            ...
            if (await _jobQueryService.IsAnyJobInProgressAsync(schedule.CollectionId, null, CancellationToken.None))
            {
                _logger.LogInfo($"Schedule will be skipped as there is already a job in progress for collection : {schedule.Collection.Name}");
                return false;
            }
