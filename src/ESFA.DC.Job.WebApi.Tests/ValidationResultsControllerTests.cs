...
// var mockKeyValuePersistenceServices = new Mock<IIndex<EnumJobType, IKeyValuePersistenceService>>();
//            mockKeyValuePersistenceServices.SetupGet(x => x[EnumJobType.IlrSubmission])
...
//            jobMetaServiceMock.Setup(x => x.GetJobById(1)).ReturnsAsync(new FileUploadJob() { JobId = 1, Ukprn = 1, JobType = EnumJobType.IlrSubmission });
